...
@pytest.mark.asyncio
async def test_session_chat_stream_forwards_narrowing_run_policy(adapter, session_db):
    session_id = session_db.create_session("policy-session", "api_server")
    captured_kwargs = {}
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                json={
                    "message": "run safely",
                    "max_iterations": 7,
                    "enabled_toolsets": ["safe", "web", "safe"],
                },
            )
            assert resp.status == 200, await resp.text()

    assert captured_kwargs["max_iterations_override"] == 7
    assert captured_kwargs["enabled_toolsets_override"] == ["safe", "web"]
...
async def test_session_chat_rejects_invalid_run_policy_before_agent_start(adapter, session_db):
    ...
    with patch.object(adapter, "_run_agent", run_agent):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                f"/api/sessions/{session_id}/chat",
                json={"message": "run", "enabled_toolsets": ["../terminal"]},
            )
            assert resp.status == 400
            payload = await resp.json()

    assert payload["error"]["code"] == "invalid_enabled_toolsets"
